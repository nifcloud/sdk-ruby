...
options = {
  ...
  #:listener_update    => [{:load_balancer_port          => 0, 
  #                         :instance_port               => 0, 
  #                         :listener_protocol           => "protocol",
  #                         :listener_load_balancer_port => 0,
  #                         :listener_instance_port      => 0,
  #                         :listener_balancing_type     => "balancingType"}],
  #:network_update     => 0
}

pp response = ncs4r.update_load_balancer(options)
