...
pp response = ncs4r.start_instances(options)

response.instancesSet.item.each do |instance|
  p instance.instanceId
  p instance.currentState.code
  p instance.currentState.name
  p instance.previousState.code
  p instance.previousState.name
end
